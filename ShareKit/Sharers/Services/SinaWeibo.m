...
#define SHKSinaWeiboConsumerKey @"3363001553"	// The consumer key
#define SHKSinaWeiboSecretKey @"e8731d292135b723203f4b07f7b6d233"		// The secret key
#define SHKSinaWeiboCallbackUrl @"http://www.joyqom.com/"	// The user defined callback url
...
- (id)init
{
	...
	{		
	    ...
	    self.requestURL = [NSURL URLWithString:@"https://api.weibo.com/oauth2/access_token"];
...
{
    [self authorizationFormSave:form];
}
- (void)authorizationFormSave:(SHKFormController *)form
{
    ...
    NSDictionary *formValues = [form formValues];
     NSString *surl = [NSString stringWithFormat:@"https://api.weibo.com/oauth2/access_token?client_id=%@&client_secret=%@&grant_type=password&username=%@&password=%@", SHKSinaWeiboConsumerKey, SHKSinaWeiboSecretKey, [formValues objectForKey:@"username"], [formValues objectForKey:@"password"]];
    NSLog(surl);
    
    NSURL  *url = [ NSURL   URLWithString :surl];
    NSMutableURLRequest  *req = [[ NSMutableURLRequest   alloc ]  initWithURL :url];
    [req  setHTTPMethod : @"POST" ];
    [req setHTTPBody:[[NSString stringWithFormat:@"client_id=%@&client_secret=%@&grant_type=password&username=%@&password=%@", SHKSinaWeiboConsumerKey, SHKSinaWeiboSecretKey, [formValues objectForKey:@"username"], [formValues objectForKey:@"password"]] dataUsingEncoding:NSUTF8StringEncoding]];
//    [req  setValue :authValue  forHTTPHeaderField : @"Authorization" ];
    ...
    NSLog ( @"status code = %d, body=%@" ,statusCode, [[[NSString alloc] initWithData:result encoding:NSUTF8StringEncoding] autorelease]);
    
    NSString *alertTitle;
        NSString *alertMassage;
    if  (statusCode !=  200 ) {
        alertTitle =  @"username or password is wrong" ;
        alertMassage =  @"username or password is wrong" ;
    } else  {
        alertTitle =  @"" ;
        alertMassage =  @" 登录成功！ " ;
    }
